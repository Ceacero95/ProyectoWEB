...
def _monitor_task(**kwargs):
    ...
    MonitoringManager().update_control_table("pdvd", "omie")

with DAG(
    'omie_pdvd_pipeline',
    default_args=default_args,
    description='Pipeline for OMIE PDVD (Demanda Programada) Data',
    schedule=None,
    ...
    t1 = PythonOperator(task_id='download_pdvd', python_callable=_download_task, provide_context=True)
    t2 = PythonOperator(task_id='process_pdvd', python_callable=_process_task, provide_context=True)
    t3 = PythonOperator(task_id='monitor_pdvd', python_callable=_monitor_task)
